...
public class ViewAttributeTransformation implements LayoutCompositeInterface {
	...
	private void build(final Composite parent) {
		...
		this.cmbMax.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent arg0) {
				...
				ARXFields field = meta.getField(fieldName);
				field.setMaximumGen(
						(cmbMax.getSelectionIndex() != (cmbMax.getItemCount() - 1) ? cmbMax.getSelectionIndex() : -1));
				meta.setField(field);
			}
		});
		...
		this.textHierarchie.addModifyListener(new ModifyListener() {
		...
		this.btnField.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				dialog.setFilterNames(new String[] { BaseMessages.getString(PKG, "System.FileType.CSVFiles") });
				if (dialog.open() != null) {

					textHierarchie.setText(
	 ...
	 * 
	 * @param fieldName
	 *            The New FieldName for Updating
	 */
