...
import org.deidentifier.arx.kettle.dialoge.ARXRiskAnalysis;
...
public class ARXPluginProcess {
	...
	public ARXPluginProcess(List<String[]> rowList,String[] fieldNames,ARXPluginMeta meta){
		...
		this.maxOutliers=meta.getMaxOutliers();
		this.meta = meta;
	}
	...
	public List<String[]> run() throws IOException{
        ...
        config.setMaxOutliers(this.maxOutliers);
        ...
        if(this.meta.isDifferentialEnabled()){
        	config.addCriterion(new EDDifferentialPrivacy(meta.getDifferentialEpsilon(),meta.getDifferentialDelta(),DataGeneralizationScheme.create(GeneralizationDegree.values()[this.meta.getDifferentialGeneralizationIndex()])));
        }
        ...
        if(this.meta.getRegion()!=null&&this.meta.getRegions(this.meta.getRegion())!=null){
        	if(temp.getPopulationSize()!=this.meta.getRegions(this.meta.getRegion()).getPopulation()){
        		populationModel=ARXPopulationModel.create(this.meta.getRegions(this.meta.getRegion()).getPopulation());
        	}
        }
        
        if(this.meta.isKmapEnabled()){
        	config.addCriterion(new KMap(this.meta.getKmap(),this.meta.getKmapSignificanceLevel(),populationModel,(this.meta.getKmapEstimatorIndex()==0?CellSizeEstimator.POISSON:CellSizeEstimator.ZERO_TRUNCATED_POISSON)));
        }
        ...
        if(this.meta.isPopulationUniquenessEnabled()){
        	config.addCriterion(new PopulationUniqueness(this.meta.getPopulationUniqueness(),this.getPopulationUniquessModel(this.meta.getPopulationUniquenessModel()),populationModel));
        }
        ...
        for(int i=0;i<this.fieldNames.length;i++){
        		 ...
        		 if(field.islDiversityEnable()){
        			 switch(field.getlDiversityVariant()){
        			 	...
        			 	case 1:
        			 		...
        			 		break;
        			 	default:
        			 		config.addCriterion(new RecursiveCLDiversity(field.getName(),field.getlDiversityC(),field.getlDiversity()));
        			 		break;
        			 }
        		 }
        	...
        	if(field.getTransformation()==0){
                ...
                MicroAggregationFunction function = getMicroAggregationFunction(field.getFunctionMicro()).createInstance(field.isMissingDataMicro());
        ...
        //TODO Find out why the result set is null (Only by K-Anonymity and CL-Diversity)
