...
public class LayoutTopLeft {
	...
	public void build(CTabFolder parent){
		...
		this.buildHIPAA(parent);
	}
	
	private void buildHIPAA(CTabFolder parent){
		CTabItem tabField = new CTabItem( parent, SWT.NONE );
	 	tabField.setText( Resources.getMessage("RiskAnalysis.26") ); 
      Composite tabFieldComp = new Composite( parent, SWT.NONE );
     
      tabFieldComp.setLayout( SWTUtil.createGridLayoutWithEqualWidth(1));
      
        new ViewRisksHIPAA(tabFieldComp,data,population);
	...
	private void buildReidentificationRisks(CTabFolder parent){
      ...
      new ViewRisksReidentificationRisks(tabFieldComp,result2,population,risk);
